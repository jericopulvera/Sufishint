...
{
    return get_form_response(false, ['error' => $error]);
}
...
function get_form_response($success, $data)
{
    ...
    $status = [];
    $status[$success ? 'FormResponse' : 'MusePHPFormResponse'] = array_merge(['success' => $success], $data);
...
function check_required_fields($form)
{
    ...
    foreach ($form['fields'] as $field => $properties) {
        ...
        if (!array_key_exists($field, $_REQUEST) || ($_REQUEST[$field] !== '0' && empty($_REQUEST[$field]))) {
            array_push($errors, ['field' => $field, 'message' => $properties['errors']['required']]);
        } elseif (!check_field_value_format($form, $field, $properties)) {
            array_push($errors, ['field' => $field, 'message' => $properties['errors']['format']]);
        }
    }

    if (!empty($errors)) {
        die(get_form_error_response(['fields' => $errors]));
    }
}
...
function get_email_body($subject, $heading, $fields, $resources)
{
    ...
    foreach ($fields as $field => $properties) {
        ...
        array_push($sorted_fields, ['field' => $field, 'properties' => $properties]);
    }
